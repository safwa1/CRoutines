...
public sealed class FlowTask : IObservable<TaskStateChangedEvent>, IProgress<double>
{
    private readonly Func<CancellationToken, Func<Task>, Task> _action;
        ...
        private readonly SemaphoreSlim _pauseSemaphore = new(1, 1); // Async pause/resume
        ...
        public FlowTask WithPriority(TaskPriority priority) { Priority = priority; return this; }
        public FlowTask After(params FlowTask[] dependencies) { _dependencies.AddRange(dependencies); return this; }
        public FlowTask Schedule(TimeSpan delay, bool repeat = false) { _delay = delay; _isRepeating = repeat; return this; }
        ...
        public async Task StartAsync()
        {
            ...
            foreach (var dep in _dependencies)
                while (dep.State != TaskState.Completed && dep.State != TaskState.Canceled && dep.State != TaskState.Faulted)
                    await Task.Delay(50);
